...
def match_pubs_titleauthor():
    ...
    def get_primary_and_secondary_awards(publication, warning1_log,warning2_log ):
        ...
        isthere_award_in_pub = False # Does this publication has listed awards?
        ...
        try:
            awards_in_pub = publication['awards']
        except:
            #print(publication)
            awards_in_pub = ['NONE']
        ...
        for x in range(len(authors_in_pub) - 1, -1, -1):
            last_author = authors_in_pub[x]
            ...
            for pi in pi_and_awards_portal.values():
                ...
                if is_same_author(last_author, pi_fname, pi_lname):  # find the 4DN member in the list of authors in the publication
                    ...
                    if len(awards_in_pub) == 1 and awards_in_pub[0] == 'NONE' and isthere_4DN_member:
                        for x in pi_awards:
                            if 'TCPA' in x:
                                primary_award.append(x)
                            else:
                                primary_award.append(x[1:-3])
                    ...
                    # If the publication has more than one award, find the primary and secondary awards
                        ...
                        for award in awards_in_pub:
                            if len(pi_awards) > 1: # if the 4DN member has more than 1 award, get both awards
                                x = 0
                ...
                if ismatch == True: # If found a match in author, stop searching in the 4DN member list
                   break
            if isthere_4DN_member == True: #if found a match in author, stop searching the Authors in pub list
                break

        #If there are not 4DN members in this publication, give a warning and award in pub becomes primary
        if isthere_4DN_member == False:
            PIs_4DN = []
            for award in awards_in_pub:
                for pi in pi_and_awards_portal.values():
        ...
        if isthere_4DN_member == True and primary_award == False:
            for x in secondary_award:
                primary_award.append(x)
            secondary_award = []
            warning2_log['no_matching_awards'][publication["fname"]] = {'awards in pub':publication['awards'], '4DN_authors':[{'name':pi['first_name'] + ' ' + pi['last_name'],'awards':pi['awards']} for pi in authors_4DN_in_pub]}
            warn2 = True
