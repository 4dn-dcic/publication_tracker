...
def map_urls_over_local_paths(ret_dict, url_map):
    if isinstance(url_map, list):
        url_map = { r[1] : r[0] for r in url_map }
    for award in ret_dict["awards"].keys():
        ret_dict["awards"][award] = [ url_map.get(r, r) for r in ret_dict["awards"][award] ]
    ret_dict["parsing_errors"] = [ url_map.get(r, r) for r in ret_dict["parsing_errors"] ]
    ret_dict["remaining"] = [ url_map.get(r, r) for r in ret_dict["remaining"] ]
    return ret_dict



def combine_output_dicts(output_dicts):
...
if __name__ == '__main__':
    ...
    main_dict = combine_output_dicts(output_dicts)

    if url_map:
        main_dict = map_urls_over_local_paths(main_dict, url_map)
